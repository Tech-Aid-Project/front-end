...
def validate_branch_and_pr_title(branch, pr_title):
    ...
    # Verify if the branch starts with 'TAFE-'
    if not branch.startswith("TAFE-"):
        print(f"The branch '{branch}' does not start with the pattern 'TAFE-'.")
        return False
    # Verify if the branch contains with 'TAFE-'
    if "TAFE-" not in pr_title:
        print(f"The PR title '{pr_title}' does not contain the pattern 'TAFE-'.")
        return False
    
    print(f"Validation successful: The branch '{branch}' and the PR title '{pr_title}' follow the 'TAFE-' pattern.")
